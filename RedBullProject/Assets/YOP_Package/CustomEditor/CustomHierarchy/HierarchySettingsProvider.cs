...
static class HierarchySettingsProvider
{
            ...
            guiHandler = (searchContext) => {
                ...
                var settings =
                    new SerializedObject(
                        AssetDatabase.LoadAssetAtPath<HierarchySettingsSO>(
                            "Assets/Resources/ScriptableObject/CustomHierarchy.asset"));
    ...
    private static void DrawIconContentBox(SerializedObject serializedObject) {
        using (new GUILayout.VerticalScope(EditorStyles.helpBox)) {
            ...
            using (new GUILayout.HorizontalScope()) {
                ...
                //Draw Ignore Icons
                using (new GUILayout.VerticalScope(EditorStyles.helpBox)) {
                    ignoreIconsFoldout = EditorGUILayout.Foldout(ignoreIconsFoldout,
                        new GUIContent("Icon's Name To Ignore",
                            "Write the name of the Icons you don't want to Show in front of the GameObject"), true);
    ...
    private static void DrawTreeBranchBox(SerializedObject serializedObject) {
        using (new GUILayout.VerticalScope(EditorStyles.helpBox)) {
            ...
            GUILayout.Space(2);
            using (new GUILayout.HorizontalScope()) {
                serializedObject.FindProperty("useTreeBranch").boolValue = GUILayout.Toggle(
                    serializedObject.FindProperty("useTreeBranch").boolValue, GUIContent.none, GUILayout.Width(15));

                GUI.enabled = serializedObject.FindProperty("useTreeBranch").boolValue;
